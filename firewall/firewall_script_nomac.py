...
def start(kwargs):
    ...
    res += __execute('iptables -A FORWARD-RIVNET -i ' + kwargs['lan_int'] + ' -p icmp -j ACCEPT')
    res += ["\n"]
        ...
        for mac in client["macs"]:
            ...
            if client["admin"]:
                ...
                if len(kwargs['forward_tcp_ports']):
                    ...
                    for a in tab:
                        res += __execute('iptables -A FORWARD-RIVNET -i ' + kwargs['lan_int'] + ' -o ' + kwargs['wan_int'] + '  -p tcp -m multiport --dports ' + ','.join(a) + ' -m state --state NEW -j ACCEPT')

                if len(kwargs['forward_udp_ports']):
                    tab = __sub(','.join(kwargs['forward_udp_ports']).split(','), 10)
                    for a in tab:
                        res += __execute('iptables -A FORWARD-RIVNET -i ' + kwargs['lan_int'] + ' -o ' + kwargs['wan_int'] + '  -p udp -m multiport --dports ' + ','.join(a) + ' -m state --state NEW -j ACCEPT')
